...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, input_mask, segment_ids, label_id, tweet_id, date, label
    ):
...
def convert_input(the_input, max_seq_length):
    ...
    # # print("INNNNSIDDDE convertinput")
    # if len(the_input.text)<2:
    #     # print(the_input.text)
    #     return None
    try:
        tokenizer = RobertaTokenizer.from_pretrained("roberta-base")
    ...
    encode_plus_tokens = tokenizer.encode_plus(
        the_input.text, padding="max_length", max_length=max_seq_length, truncation=True
    )
...
def transform_inputs_to_tfrecord(inputs, output_file, max_seq_length):
    # print("INNNNSIDDDE transform_inputs_to_tfrecord")
    records = []
    ...
    for (input_idx, the_input) in enumerate(inputs):
        ...
        try:
            ...
            if features==None:
                print("input_idx None",input_idx,the_input.text,the_input.tweet_id,the_input.label)
                continue
       
            if input_idx<5:
                print("5 CONVERT IS DONE")
            if input_idx>150 and input_idx<160:
                print("GREATER 1050 DATA")
        
        
            all_features = collections.OrderedDict()

        # Create TFRecord With input_ids, input_mask, segment_ids, and label_ids
            all_features["input_ids"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=features.input_ids)
        )
            all_features["input_mask"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=features.input_mask)
        )
            all_features["segment_ids"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=features.segment_ids)
        )
            all_features["label_ids"] = tf.train.Feature(
            int64_list=tf.train.Int64List(value=[features.label_id])
        )
        ...
        # Create Record For Feature Store With All Features
            ...
            print("Exception: {}".format(e))
            print("input_idx",input_idx,the_input.text,the_input.tweet_id,the_input.label)
            break
    print("CONVEEERT DONE")
...
def transform_csv_to_tfrecord(
    file, max_seq_length, balance_dataset, prefix, feature_group_name
):
    ...
    print("INNNNSIDDDE transform_csv_to_tfrecord")
    ...
    print("GOING INNNNSIDDDE transform_inputs_to_tfrecord FOR TRAIN")
    train_records = transform_inputs_to_tfrecord(
        train_inputs,
        "{}/part-{}-{}.tfrecord".format(
            train_data, args.current_host, filename_without_extension
        ),
        max_seq_length,
    )
    print("GOING INNNNSIDDDE transform_inputs_to_tfrecord FOR VALIDATION")
    validation_records = transform_inputs_to_tfrecord(
        validation_inputs,
        "{}/part-{}-{}.tfrecord".format(
            validation_data, args.current_host, filename_without_extension
        ),
        max_seq_length,
    )
    print("GOING INNNNSIDDDE transform_inputs_to_tfrecord FOR TEST")
    test_records = transform_inputs_to_tfrecord(
        test_inputs,
        "{}/part-{}-{}.tfrecord".format(
            test_data, args.current_host, filename_without_extension
        ),
        max_seq_length,
    )
...
def process(args):
    ...
    for dr in dirs_input:
        print("input-files-stuc", dr)
    input_files = args.input_data + "/" + "twitter_sentiment_data.csv"
    print("input-files ", input_files)
    transform_csv_to_tfrecord(
        input_files,
        max_seq_length=args.max_seq_length,
        balance_dataset=args.balance_dataset,
        prefix=args.feature_store_offline_prefix,
