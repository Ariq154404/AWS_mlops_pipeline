...
def process(args):
    ...
    print(
        """IBangladesh did not cause climate change, so the country does not need Ã¢â‚¬Å“aidÃ¢â‚¬ï†; instead it needs compensation for theÃ¢â‚¬Â¦ https://t.co/ecGXgRNROP""",
        predict("""Bangladesh did not cause climate change, so the country does not need Ã¢â‚¬Å“aidÃ¢â‚¬ï†; instead it needs compensation for theÃ¢â‚¬Â¦ https://t.co/ecGXgRNROP""", model, args.max_seq_length),
    )
...
#     )
    ...
    test_data_path=input_files
    print("Using only {} to evaluate.".format(test_data_path))
    df_test_reviews = pd.read_csv(test_data_path)[
        ["message", "sentiment"]
    ]

    df_test_reviews = df_test_reviews.sample(n=10)
    df_test_reviews.shape
    df_test_reviews.head()

    y_test = [predict(message, model, args.max_seq_length) for message in df_test_reviews["message"]]
    y_test
